...
export function FaceReset({ isOpen, userId, onSuccess, onCancel }: FaceResetProps) {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      setError('ไม่สามารถเข้าถึงกล้องได้ กรุณาเปิดกล้องและอนุญาตสิทธิ์การเข้าถึงกล้องในเบราว์เซอร์')
    }
  }, [])
  ...
  const handleFinalVerification = useCallback(async () => {
    ...
    try {
      ...
      // ส่งไปยืนยันกับเซิร์ฟเวอร์เพื่อขอ reset token
      const response = await fetch('/api/auth/face-verify', {
        ...
        body: JSON.stringify({
          ...
          forPasswordReset: true // พิเศษ: ระบุว่าเป็นการขอลิงก์รีเซ็ตรหัสผ่าน
        })
      })

      const result = await response.json()

      if (response.ok && result.isMatch && result.resetToken) {
        onSuccess(result.resetToken)
      } else {
        setError(result.message || 'ใบหน้าไม่ตรงกับข้อมูลที่ลงทะเบียนในระบบ')
        handleRestartRef.current?.()
      }

    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'เกิดข้อผิดพลาดในการตรวจสอบใบหน้า')
      handleRestartRef.current?.()
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-50">
      <Card className="p-8 w-full max-w-lg bg-white relative">
        <button 
          onClick={onCancel}
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">รีเซ็ตรหัสผ่านเพื่อความปลอดภัย</p>
          <p className="text-lg text-gray-600 font-semibold mt-2">
            {isPoseVerified ? 'ยืนยันใบหน้าสำเร็จ...' : currentPose ? `${currentPose.title}` : 'กำลังเตรียมท่า...'}
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          />
          {error && (
            <p className="text-sm text-red-600 text-center font-medium mt-2">{error}</p>
          )}
        </div>
      </Card>
    </div>
