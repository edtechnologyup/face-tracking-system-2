...
const regex = /const startTracking = useCallback\(async \(\) => \{[\s\S]*?\}, \[createTrackingSession, initializeCamera, initializeHybridDetectors, startHybridTracking, startRecording\]\)/;

const newCode = `const startTracking = useCallback(async () => {
    setIsLoading(true)
    try {
...
if (regex.test(content)) {
  fs.writeFileSync(path, content.replace(regex, newCode));
